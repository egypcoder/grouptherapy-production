...
const chatColors = [
  ...
  { text: "text-indigo-500", bg: "bg-indigo-500/10", border: "border-indigo-500/20" },
...
function getUserColor(username: string): { text: string; bg: string; border: string } {
  ...
  return (
    chatColors[index] ?? { text: "text-orange-500", bg: "bg-orange-500/10", border: "border-orange-500/20" }
  );
}
    ...
    <Card className="h-full flex flex-col overflow-hidden border-border">
      ...
      <CardContent className="flex-1 flex flex-col p-0 overflow-hidden">
        <ScrollArea className="flex-1 px-4 py-4">
          <div className="space-y-3 pb-4" ref={scrollRef}>
            <AnimatePresence>
              {messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    {!msg.isSystem && (
                      <Avatar className={cn("h-8 w-8 flex-shrink-0", userColor.border, "border")}>
                        <AvatarFallback
                          className={cn(userColor.bg, userColor.text, "font-medium")}
                        >
                    ...
                    <div
                      className={cn("flex-1 min-w-0", msg.isSystem && "text-center")}
                    >
                      {!msg.isSystem && (
                        <p
                          className={cn(
